...
//		resp.getWriter().write("<li>POST http://localhost:8080/chamados/ (application/json)</li>");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)	throws ServletException, IOException {
		try {
			...
			if (req.getParameter("tipo_chamado") != null &&
				...
				Chamado chamado = new Chamado(descricao, new Date(), Chamado.STATUS_OPEN, latitude, longitude, new TipoChamado(codigoTipoChamado, getDescricao(codigoTipoChamado)));
	...
	private String getDescricao(String codigoTipoChamado) {
		for (IServicoChamado iServicoChamado : servicoChamados) {
			if(iServicoChamado.getSigla().equals(codigoTipoChamado)) {
				return iServicoChamado.getDescricao();
			}
		}
		return "";
	}
